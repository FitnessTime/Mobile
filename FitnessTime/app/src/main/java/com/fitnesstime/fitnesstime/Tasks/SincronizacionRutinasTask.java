...
public class SincronizacionRutinasTask extends AsyncTask<Void,Void,String> {
    ...
    protected String doInBackground(Void... params) {
        ...
        {
            Gson gson = new Gson();
            String rutinasDTO = gson.toJson(new ServicioRutina().getNoSincronizadas(), List.class);
            ResponseHelper response = new ServicioRutina().sincronizarAPI(rutinasDTO);
            if(response.getCodigo()==200)
            {
                new ServicioRutina().sincronizarRutinas(gson.fromJson(response.getMensaje(), List.class));
            }
        }
        return "";
    }
